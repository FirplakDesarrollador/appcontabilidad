...
async function findValue() {
    ...
    console.log('Searching for values 4151963.55 or 2040554.88 in first 50 items...');
    const itemsRes = await client.api(`/sites/${siteId}/lists/${listId}/items?expand=fields&top=50`).get();

    for (const item of itemsRes.value) {
        const fields = item.fields;
        for (const [key, val] of Object.entries(fields)) {
            const strVal = String(val);
            if (strVal.includes('4151963') || strVal.includes('2040554') || strVal.includes('2025380')) {
