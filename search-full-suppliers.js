...
const options = {
  ...
  path: `/rest/v1/Registro_Facturas?select=*&Proveedor=ilike.*SODIMAC*&limit=5`,
  headers: {
    'apikey': supabaseKey,
...
https.get(options, (res) => {
  ...
  res.on('end', () => {
    console.log('RESULTS FOR SODIMAC:', data);
  });
});

const options2 = {
  hostname: supabaseUrl,
  path: `/rest/v1/Registro_Facturas?select=*&Proveedor=ilike.*ALMACENES%20EXITO*&limit=5`,
  headers: {
    'apikey': supabaseKey,
    'Authorization': `Bearer ${supabaseKey}`
  }
...
https.get(options2, (res) => {
  ...
  res.on('data', (chunk) => { data += chunk; });
  res.on('end', () => {
    console.log('RESULTS FOR EXITO:', data);
  });
});
