...
export default function InvoicesPage() {
    ...
    const handlePreview = async (invoice: any) => {
        try {
            ...
            const apiUrl = `/api/externo/factura-viventta/${invoice.id}/download?file=${encodeURIComponent(fileName)}`;
    ...
    const handleUploadManualAttachments = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/facturas-viventta/${selectedInvoice.id}/adjuntos`, {
                method: "POST",
                body: formData,
            });
            const data = await res.json();
    ...
    const fetchInvoices = async (refresh: boolean = false) => {
        try {
            ...
            const response = await fetch(`/api/facturas-viventta/all?${params.toString()}`);
    ...
    const fetchHistory = async (reset: boolean = false, overrideLimit?: number | 'all') => {
        ...
        try {
            ...
            const response = await fetch(`/api/facturas-viventta/all?processed=true&offset=0&limit=${actualLimit}`); 
            const data = await response.json();
    ...
    const handleAction = async (action: string, field: string = 'Aprobacion_Doliente') => {
        ...
        try {
            const res = await fetch("/api/facturas-viventta/update-status", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    itemId: selectedInvoice.id,
    ...
    const colDefs = useMemo(() => [
        ...
        { headerName: 'Datos adjuntos', field: 'adjuntos_url', width: 150, filter: false, sortable: false, cellRenderer: (p: any) => { const pdfUrl = p.data?.documentos || p.data?.fp; return <div className="h-full flex items-center">{pdfUrl ? <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition-all border border-blue-100/50" title="Ver Documento Adjunto"><FileText className="h-3.5 w-3.5" /><span className="text-[10px] font-black uppercase tracking-tight">Ver Adjunto</span></a> : <span className="text-[10px] text-gray-300 font-medium italic">Sin adjuntos</span>}</div>; } },
                ...
                <header className="h-20 bg-white/80 backdrop-blur-xl border-b border-gray-100 flex items-center justify-between px-8 sticky top-0 z-10 w-full">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-xl font-bold text-gray-800 tracking-tight">Aprobación de Facturas <span className="text-[#254153]">Viventta</span></h1>
                        </div>
                    </div>
