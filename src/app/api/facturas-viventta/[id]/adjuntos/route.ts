...
export async function POST(
    ...
    try {
        ...
        if (!currentInvoice) {
            return NextResponse.json({ error: "Factura Viventta no encontrada en Supabase" }, { status: 404 });
        }

        const folderName = `viventta_adjuntos/${sanitizePathPart(invoiceNumber || id)}`;
        ...
        console.error("Error uploading Viventta invoice attachments:", error);
...
export async function DELETE(
    ...
    try {
        ...
        console.error("Error deleting Viventta invoice attachment:", error);
        return NextResponse.json({ error: error.message }, { status: 500 });
    }
}
