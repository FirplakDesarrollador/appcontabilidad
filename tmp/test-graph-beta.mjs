...
try {
    ...
    if (attachments.value.length === 0) {
        ...
        console.log(`Found attachment: ${att.name} (ID: ${att.id})`);
        
        // Let's try to get its content using beta graph api
        try {
            const fileStream = await client.api(`/sites/${siteId}/lists/${listId}/items/${itemId}/attachments/${att.id}/$value`).version('beta').getStream();
            ...
            console.log(`SUCCESS! Downloaded file via BETA Graph API. Size: ${buffer.length} bytes.`);
        } catch(e) {
            console.log("BETA Error downloading attachment content via graph:", e.message);
        }
    }
} catch (e) {
