...
async function check() {
    ...
    // Extract valid Nro_Factura
    const spInvoices = new Set();
    for (const item of spItems) {
        if(item.fields && item.fields.Nro_Factura) {
            spInvoices.add(item.fields.Nro_Factura.trim());
        }
    }
    const uniqueInvoices = Array.from(spInvoices).filter(Boolean);
    console.log(`Found ${uniqueInvoices.length} unique invoice numbers in SP.`);
    ...
    let sapMatchCount = 0;
    const chunkSize = 20; // 20 invoices per query to avoid URI too long
    
    console.log('Querying SAP...');
        ...
        try {
            ...
            if (invoicesRes.ok) {
                ...
                if (data.value && data.value.length > 0) {
                    // SAP might return multiple records for the same NumAtCard
                    const found = new Set(data.value.map(v => v.NumAtCard));
                    sapMatchCount += found.size;
                }
            }
        } catch (e) {
            console.error('Error fetching chunk from SAP:', e.message);
        }
        
        if (i % 500 === 0) console.log(`Processed ${i} / ${uniqueInvoices.length}`);
    }
    
    console.log(`\nRESULTADO FINAL: Hay ${sapMatchCount} facturas de SharePoint que están en SAP.`);
}
