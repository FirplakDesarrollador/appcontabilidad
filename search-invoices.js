...
https.get(options, (res) => {
  ...
  res.on('end', () => {
    console.log('SEARCH RESULTS FOR 325284:', data);
  });
});

const options2 = {
  hostname: supabaseUrl,
  path: `/rest/v1/Registro_Facturas?select=Nro_Factura&Nro_Factura=ilike.*1264308*`,
  headers: {
    'apikey': supabaseKey,
    'Authorization': `Bearer ${supabaseKey}`
  }
...
https.get(options2, (res) => {
  ...
  res.on('data', (chunk) => { data += chunk; });
  res.on('end', () => {
    console.log('SEARCH RESULTS FOR 1264308:', data);
  });
});
